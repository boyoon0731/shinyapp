...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h3("Filters"),
      sliderInput(inputId = "VORP", label = "Player VORP rating at least", min = -3, max = 10, c(0, 10)),
      selectInput("Team", "Team", unique(players$Team), multiple = TRUE),
      h3("Plot options"),
      selectInput("variable", "Variable", c("VORP", "Salary", "Age", "Height", "Weight"), "Salary"),
    ...
    mainPanel(
      strong("There are ", textOutput("num_players", inline = TRUE)," players in the dataset"),
      plotOutput("nba_plot"),
